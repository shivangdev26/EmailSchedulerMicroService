...
const fetchAllDatabases = async (retries = 3) => {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      lastError = err;
      logger.warn(
        `Alert Fetch databases attempt ${i + 1}/${retries} failed:`,
        err.message,
      );
      if (i < retries - 1) {
  ...
  logger.error("Alert Error fetching databases after retries", {
...
const addRepeatJob = async (payload, cron, jobId) => {
  if (scheduledJobCache.get(jobId) === cron) {
    logger.debug(`Alert Job already scheduled with same cron, skipping`, {
      jobId,
      cron,
    });
    return;
  ...
  for (const j of existing) {
    if (j.key && j.key.includes(jobId)) {
      logger.info(`Removing old repeatable alert job`, {
        jobId,
        pattern: j.pattern,
      });
...
const pollAlertScheduler = async () => {
  ...
  try {
    ...
    logger.error("Alert Scheduler error", {
      error: err.message,
      stack: err.stack,
    });
  } finally {
