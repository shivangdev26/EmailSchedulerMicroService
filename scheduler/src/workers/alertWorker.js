...
const startAlertWorker = () => {
  ...
  workerConnection.on("error", (err) =>
    logger.error("AlertWorker Redis error", { error: err.message }),
  );
  workerConnection.on("connect", () =>
    logger.info("AlertWorker Redis connected"),
  );
    ...
    async (job) => {
      try {
        if (job.name === "send-alert") {
          ...
          logger.info("=== send-alert job picked up ===", {
            jobId: job.id,
            db,
          ...
          const eligibleUsers = (alertSetup.m_alert_setup_user || []).filter(
            (u) => u.alert === "Y",
          );

          if (eligibleUsers.length === 0) {
            logger.info("No eligible users with alert='Y' found.", {
              alertSetupId: alertSetup.id,
            });
            return;
          }
          ...
          for (const user of eligibleUsers) {
            ...
            if (user.email === "Y") {
              const emailAddress = await resolveUserEmail(
                user.user_id,
                db,
                token,
              );
        ...
        logger.error("Error processing alert job", {
          jobId: job.id,
          error: err.message,
          stack: err.stack,
        });
